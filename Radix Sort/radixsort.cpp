...
int main(int argc, char *argv[])
{
  ...
  err = clEnqueueWriteBuffer(cv.commands, g_temp, true, 0, sizeof(int)*n,
           temp, 0, NULL, NULL);
  CHK_ERR(err);
  ...
  for (int32_t k = 0; k < 32; k++) {
    //Compute entries with set lsb 
    //for (int32_t i = 0; i < n; i++) {
      //temp[i] = (in[i] >> k) & 0x1;
    //}

    //Compute indices of elements with 0 in lsb 
    rsort_scan(cv.commands, cv.context,
      kernel_map[scan_name_str], kernel_map[update_name_str],
      g_in, g_zeros, 0, k, n);
    //Compute indices of elements with 1 in lsb
    rsort_scan(cv.commands, cv.context,
      kernel_map[scan_name_str], kernel_map[update_name_str],
      g_in, g_ones, 1, k, n);

    //Scatter partially ordered outputs 
    rsort_reassemble(cv.commands, cv.context,
      kernel_map[reassemble_name_str],
      g_in, g_out, g_temp, g_zeros, g_ones, n);

    //Not an in place sort, need a pointer swap 
    g_temp = g_in;
    g_in = g_out;
    g_out = g_temp;
  }
...
void rsort_reassemble(cl_command_queue &queue,
  ...
  err = clSetKernelArg(reassemble_kern, 2, sizeof(cl_mem), &temp);
  CHK_ERR(err);

  err = clSetKernelArg(reassemble_kern, 3, sizeof(cl_mem), &zeros);
  CHK_ERR(err);

  err = clSetKernelArg(reassemble_kern, 4, sizeof(cl_mem), &ones);
  CHK_ERR(err);;
  ...
  if(left_over > 1)
    {
      /* Recursively perform scan if needed */
      rsort_reassemble(queue,context,reassemble_kern,
        in,out,temp,zeros,ones,left_over);
    }
