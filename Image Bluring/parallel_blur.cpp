...
// Parallelized & Vectorized Blur
void my_blur(float* out, int n, float* frame, int* radii, int nthr){  
  omp_set_num_threads(nthr);
  int r, c, rd, num, r2, c2;
  float avg;

  //Parallelize
  #pragma omp parallel for private(c,rd,r2,c2) shared(out,frame,radii) collapse(2) reduction(+:avg,num) schedule(guided)
  for(r=0; r<n; r++){
    for(c=0; c<n; c++){
      rd = radii[r*n+c];
      num = 0;
      ...
      for(r2=max(0,r-rd); r2<=min(n-1,r+rd); r2++){
        //Vectorize 
        for(c2 = max(0,c-rd); c2<min(((n-1)/4)*4,((c+rd)/4)*4) && c2-max(0,c-rd)<(2*rd/4)*4; c2+=4){
          frame_vector = _mm_loadu_ps(frame+(r2*n+c2));
